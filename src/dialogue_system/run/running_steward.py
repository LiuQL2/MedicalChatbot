...
class RunningSteward(object):
    ...
    def simulate(self, agent, episodes, train=False):
        ...
        for index in range(0, episodes,1):
            ...
            if result["success_rate"] >= self.best_result["success_rate"] and result["success_rate"] > dialogue_configuration.SUCCESS_RATE_THRESHOLD:
    ...
    def simulation_epoch(self,index):
        ...
        print("%3d simulation success rate %s, ave reward %s, ave turns %s" % (index,res['success_rate'], res['average_reward'], res['average_turn']))
        return res
