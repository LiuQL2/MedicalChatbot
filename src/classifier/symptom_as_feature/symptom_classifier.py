...
class SymptomClassifier(object):
    ...
    def _build_model(self):
        ...
        The model is fully connected network with the softmax as output.
        :return:
        """
    ...
    def train_tf(self):
        """
        Training model building with tensorflow.
        :return:
        """
    ...
    def evaluate_tf(self):
        """
        Evaluating the tensorflow-based model using test data set.
        :return:
        """
        ...
        for right in result:
            if right: count += 1
        print("accuracy of fully connected network in tf:", float(count)/len(result))
    ...
    def _prepare_data_set(self):
        ...
        data_set = {
            "train":[], # Traning data stored as batch which has the form {"x":[],"y":[]}.
            "test":{"x":[],"y":[]}
        }
        batch = {
            "x":[],
    ...
    def train_sklearn_svm(self):
        """
        训练scikit learn 中的svm，需要进行数据格式的转换
        :return:
        """
        self.clf = svm.SVC(decision_function_shape='ovo') # 多分类
        Xs = []
        Ys = []
        for index in range(0, len(self.data_set["train"]), 1):
            batch = self.data_set["train"][index]
            Xs = Xs + batch["x"]
            Ys = Ys + list(np.argmax(batch['y'],axis=1))
        self.clf.fit(X=Xs, y=Ys)

    def evaluate_sklearn_svm(self):
        """
        评估scikit learn 中的svm
        :return:
        """
        batch = self.data_set["test"]
        y = np.argmax(batch['y'],axis=1)
        predict = self.clf.predict(X=batch["x"])
        count = 0
        for index in range(0,len(predict),1):
            if predict[index] == y[index]:
                count += 1
        print("accuracy of svm in sklearn:", float(count)/len(predict))